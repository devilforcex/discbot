...
DrusaBoT е native Windows Discord music bot, заключен към:
...
- Inno Setup script: `scripts/windows/DrusaBoT.iss`.
- Fixed path enforcement: `E:\DrusaBoT` only.
- Lavalink config uses the YouTube source plugin and keeps built-in YouTube disabled.
- Optional cookies use `E:\DrusaBoT\ytcookies.txt` plus `plugins.youtube.cookieFile` in `application.yml`.

...
class MusicCommands(commands.Cog):
    ...
    async def pause(self, ctx: commands.Context) -> None:
        ...
        if player.paused:
            embed = build_error_embed(description="Playback is already paused. Use `!resume`.")
            await ctx.send(embed=embed)
            return
        ...
        embed = discord.Embed(
            title="⏸️ Paused",
            description="Playback has been paused. Use `!resume` to continue.",
    ...
    async def resume(self, ctx: commands.Context) -> None:
        ...
        if not player.paused:
            embed = build_error_embed(description="Playback is not paused.")
            await ctx.send(embed=embed)
            return
    ...
    async def skip(self, ctx: commands.Context) -> None:
        ...
        # Store current track info for embed
        current_title = player.last_track.title if player.last_track else "Unknown"
    ...
    async def stop(self, ctx: commands.Context) -> None:
        ...
        await player.stop()
        self.bot.queue_manager.clear(ctx.guild.id)
    ...
    async def disconnect(self, ctx: commands.Context) -> None:
        ...
        if not player:
            embed = build_error_embed(description="Bot is not connected to a voice channel.")
            await ctx.send(embed=embed)
            return
    ...
    async def shuffle(self, ctx: commands.Context) -> None:
        ...
        if self.bot.queue_manager.is_empty(ctx.guild.id):
            embed = build_error_embed(description="The queue is empty.")
            await ctx.send(embed=embed)
            return

        self.bot.queue_manager.shuffle(ctx.guild.id)
    ...
    async def loop(self, ctx: commands.Context, mode: str = "none") -> None:
        ...
        emoji = mode_emojis.get(loop_mode, "🔄")
    ...
    async def volume(self, ctx: commands.Context, volume: int = 50) -> None:
        ...
        if volume < 0 or volume > 100:
            embed = build_error_embed(description="Volume must be between 0 and 100.")
            await ctx.send(embed=embed)
            return

        await player.set_volume(volume)

        # Save volume to guild settings
        try:
            guild_settings.set(
                str(ctx.guild.id),
                self.bot.config.database_path,
            ...
            logger.debug("Failed to save volume setting: %s", e)
    ...
    async def favorite(self, ctx: commands.Context) -> None:
        """Save the currently playing track to favorites.
        ...
        if success:
            embed = discord.Embed(
                title="⭐ Favorited",
                description=f"Added **{track.title}** to your favorites.",
                color=discord.Color.gold(),
            )
        else:
            embed = discord.Embed(
                title="❌ Already Favorited",
                description=f"**{track.title}** is already in your favorites.",
                color=discord.Color.orange(),
            )

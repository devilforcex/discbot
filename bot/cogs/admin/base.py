...
from bot.core.services.auth import resolve_user_id as _resolve, is_owner
...
async def log_audit(action: str, target_user_id: str, target_username: str, moderator_id: str, db_path: str):
    try:
        ...
        conn.commit()
        logger.info("Audit: %s | target=%s (%s) | moderator=%s", action, target_username, target_user_id, moderator_id)

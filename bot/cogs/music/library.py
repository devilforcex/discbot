...
class LibraryCog(commands.Cog):
    ...
    async def playlists(self, ctx):
        ...
        if not playlists:
            await ctx.send(embed=build_error_embed(description="You have no playlists. Create one with `!playlist_create <name>`."))
            return
        embed = discord.Embed(
    ...
    async def playlist_show(self, ctx, playlist_id: str, page: int = 1):
        ...
        if not playlist:
            await ctx.send(embed=build_error_embed(description="Playlist not found."))
            return
        if not playlist.get("tracks"):
            await ctx.send(embed=build_error_embed(description="This playlist has no tracks."))
            return
    ...
    async def playlist_play(self, ctx, playlist_id: str):
        ...
        async with ctx.typing():
            ...
            try:
                ...
                return
            if not player:
                await ctx.send(embed=build_error_embed(description="Failed to connect to voice."))
                return

...
def add_track(
    ...
    conn.commit()
    cur.execute("UPDATE playlists SET updated_at = CURRENT_TIMESTAMP WHERE playlist_id = ?", (playlist_id,))
    conn.commit()
    logger.info("Added track '%s' to playlist %s (pos %d)", title, playlist_id, next_pos)
    return True


def remove_track(playlist_id: str, position: int, user_id: str, db_path: str = "data/musicbot.db") -> bool:
    conn = get_connection(db_path)
    cur = conn.cursor()

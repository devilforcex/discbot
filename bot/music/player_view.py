...
# custom_id prefix for persistent views
CID_PREFIX = "mb"
...
class PlayerView(discord.ui.View):
    ...
    def _btn(
        ...
        button = discord.ui.Button(
            emoji=emoji,
            style=style,
            custom_id=f"{CID_PREFIX}:{action}:{guild_id}",
            row=row,
        )
    ...
    async def _handle(self, interaction: discord.Interaction, action: str) -> None:
        ...
        if not guild_id:
            await interaction.response.send_message(
                f"{EMOJI['error']} Could not resolve guild.",
                ephemeral=True,
            )
            return
        ...
        if self._on_cooldown(interaction.user.id):
            await interaction.response.send_message(
                f"{EMOJI['error']} Slow down a second.",
                ephemeral=True,
            )
            return
        ...
        if not isinstance(user, discord.Member):
            await interaction.response.send_message(
                f"{EMOJI['error']} Guild members only.",
                ephemeral=True,
            )
            return

        # Queue is special: show ephemeral embed, no full defer refresh needed first
        if action == "queue":
            ...
            if not queue_list and not current:
                await interaction.followup.send(
                    f"{EMOJI['error']} The queue is empty.",
                    ephemeral=True,
                )
                return
...
def make_persistent_view(bot, guild_id: int = 0) -> PlayerView:
    """Create a view instance for bot.add_view (guild_id 0 matches template ids)."""
